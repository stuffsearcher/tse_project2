...
data_df = preparation.prepare_main_data(data_name="data", annual=True)
turbine_data = preparation.prepare_turbine_data(data_name="turbine_data")


def calculate_annual_yield(wind_series: pd.core.series.Series, turbine_name: str):
    ...
    theoretical_maximum = max_power * 8760
    capicity_factor = (total_energy / theoretical_maximum) * 100

    print(
        f"Total energy yield/capacity factor for {turbine_name} in one year: {total_energy/1e6:.2f} GWh / {capicity_factor:.2f}  "
    )
    return total_energy


for tb_name in turbine_data["Turbine"].unique():
    calculate_annual_yield(turbine_name=tb_name, wind_series=data_df["speed"])
